  ...
  return (
    <div className="seq-wrap" style={{ flex: 1, overflow: 'auto', position: 'relative', background: '#f8fafc', border: '1px solid #eef2f7', borderRadius: 8, padding: 8 }}>
      ...
      >
        ...
        <text x={FLOW_CLIENT_X} y={FLOW_HEADER_H - 14} textAnchor="middle" fill="#1d4ed8" fontWeight="bold">
          {conv.client}
        </text>
        <text x={FLOW_SERVER_X} y={FLOW_HEADER_H - 14} textAnchor="middle" fill="#c2410c" fontWeight="bold">
          {conv.server}
        </text>
        {/* 两条垂直生命线:贯穿全图,报文在其间水平流动 */}
        <line x1={FLOW_CLIENT_X} y1={FLOW_HEADER_H} x2={FLOW_CLIENT_X} y2={layout.height - 8} stroke="#cbd5e1" strokeDasharray="4 4" />
        <line x1={FLOW_SERVER_X} y1={FLOW_HEADER_H} x2={FLOW_SERVER_X} y2={layout.height - 8} stroke="#cbd5e1" strokeDasharray="4 4" />
        ...
        {layout.rows.map((r, i) => {
          ...
          const x1 = r.dir === 'b2a' ? FLOW_SERVER_X : FLOW_CLIENT_X
          const x2 = r.dir === 'b2a' ? FLOW_CLIENT_X : FLOW_SERVER_X
          const y = yOf(i)
          return (
            <g
              key={r.number}
            ...
            >
              ...
              <rect x={2} y={y - layout.rowHeight / 2} width={layout.width - 4} height={layout.rowHeight} fill={rowBg} rx={3}>
                <title>{`${DIR_LABEL[r.dir]} · ${r.label}${r.anomaly ? ' · ⚠ tcp分析标记' : ''}`}</title>
              </rect>
              {/* 中性方向:画短横线(两端都不确定,不指令断言方向) */}
              {r.dir === 'neutral' ? (
                <line x1={FLOW_CLIENT_X + 20} y1={y} x2={FLOW_SERVER_X - 20} y2={y} stroke={stroke} strokeWidth={1.6} strokeDasharray="2 3" />
              ) : (
                <>
                  {/* 发送方圆点:报文从这条生命线发出 */}
              ...
              {/* 行内标注:#帧号 协议概要 · 长度 */}
              <text className="flow-label" x={(FLOW_CLIENT_X + FLOW_SERVER_X) / 2} y={y + 3.5} textAnchor="middle" fill={stroke} fontSize={10}>
                {r.label}
              </text>
            </g>
          )
