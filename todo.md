...
> 更新:2026-08-27(第 3 次) · 依据 `docs/specs/2026-08-25-pUI-报文分析能力升级计划.md`(M0-M7 路线)
...
| 项 | 值 |
|---|---|
| 前端测试 | 400 用例全绿(`npx vitest run`,53 文件) |
...
M5 剩余(检测器已有,按优先级):
- [ ] RST/SYN 重传接入会话标注(RST 已有 'rst' 类型,SYN 重传待接入 syn-retransmission 类型)
- [ ] RTT 分位数/variation、窗口变化统计、完整 Capture Quality(截断/丢包统计)
