...
- **场景**:A 访问 B,你与同事分别在两端抓包——把两份抓包各自独立解析,按五元组自动配对会话,按绝对时间(frame.time_epoch)对齐时间线(B 侧时钟偏移可读),逐会话对比
- **差异呈现**:报文数/字节差异、事件差异(仅 A 有 / 仅 B 有 / 两侧均有)、合并时间线(每行标注「仅A见到 / 仅B见到 / 两侧均见」)
- **结论辅助**:仅单侧观察到丢包类事件时,提示缺失可能发生在两点之间的路径段(观察层限定措辞:不构成断言,对侧抓包漏包亦可产生同样现象);两侧均见的现象标记为「横跨两点」
- **红线**:每侧独立走完整分析链路,只对齐与对比结论——绝不跨侧合并字节或重组序列空间(时钟偏移与各侧漏包会使合并结论失真)
- **导出**:对照报告 Markdown / 离线 HTML,与既有证据报告同风格(全转义、确定性)
...
5. **双点对照**:选中会话后点「⇄ 双点对照」,载入同事在另一观测点抓的文件(或用 dual-a/dual-b 示例体验),查看对齐/差异/结论与时间线,导出对照报告
6. **导出**:时序图 PNG;会话报告(格式选择 Markdown / Word / PDF);对照页「导出报告 / 导出证据 JSON / 导出 HTML」
7. **多视角**:会话 / 主机 / 摘要 / 拓扑;摘要页直方图点击桶即下钻
...
| `dual-a` / `dual-b` | 双观测点对照演示:同一条流两端各抓一份,B 侧时钟快 1.5s,丢包只发生在 A→B 路径上(A 侧重传形态 / B 侧缺口形态) |
