...
> 更新:2026-08-27(第 2 次) · 依据 `docs/specs/2026-08-25-pUI-报文分析能力升级计划.md`(M0-M7 路线)
...
| 项 | 值 |
|---|---|
| 前端测试 | 379 用例全绿(`npx vitest run`,51 文件) |
...
- [ ] **Tauri 桌面冒烟**:真实 VDI 抓包全流程(打开→会话→故障分析→切换事件→播放/暂停/单步→跳包→返回→恢复→对向视图→窄窗双标签)
- [ ] 导出(PNG/叙述)与对照页的关系(对照页是否可导出/不入报告证据的口径确认 —— 需产品决策)
