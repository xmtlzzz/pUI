 ...
 *
 *  推导不变量(审计 P3 #9 建议写成可见):0x6000_0000 = 1.5GiB 是「真实 TCP 传输
 *  落后/领先量的物理上界」与「2^31 不可定序区」之间的折中 ——
 *  - 真实长传输单方向累计发送不可能在单次观察间隔内领先/落后超过对端最大通告
 *    窗口(实际 ≤1GB 级,TSO/GRO 巨型帧链路可更大),1.5GiB 留有余量;
 *  - 必须 < 2^31(0x8000_0000):超过半空间即落入 seqDiff 不可定序区,
 *    候选带至少两个、裁决退化 —— 1.5GiB 保持对称余量在可定序区内。 */
